...
public:
    Spliterator(string const src, char split, bool is_terminated)
        : m_src(src), m_split(split), m_is_terminated(is_terminated)
    {}
    
    std::optional<string> next() {
        if (m_seek_pos == string::npos) {
            return std::nullopt;
        }
    ...
    std::optional<std::pair<string, string>> next_pair() {
        auto first = next();
        if (!first.has_value()) {
            return std::nullopt;
        }
